...
<body class="bg-[#FAF9F5] text-stone-800 antialiased min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div class="sm:mx-auto sm:w-full sm:max-w-md text-center">
        ...
        <p class="mt-2 text-center text-sm text-stone-500 font-light">
            ...
            <a href="{{ route('login') }}" class="font-medium text-amber-700 hover:text-amber-800 transition">Log In Here</a>
        </p>
    </div>
        ...
        <div class="bg-white py-10 px-4 shadow-sm border border-stone-200/60 sm:rounded-3xl sm:px-10">
            <form method="POST" action="{{ route('register') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus autocomplete="name" class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 placeholder:text-stone-400 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm sm:leading-6 px-4">
                        @if ($errors->has('name'))
                            <p class="mt-2 text-red-600 text-xs font-medium">{{ $errors->first('name') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input id="email" name="email" type="email" value="{{ old('email') }}" required autocomplete="username" class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 placeholder:text-stone-400 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm sm:leading-6 px-4">
                        @if ($errors->has('email'))
                ...
                <div class="pt-4 text-center border-t border-stone-100 flex flex-col gap-2">
                    <a href="{{ route('home') }}" class="text-xs bg-stone-50 hover:bg-stone-100 text-stone-600 transition font-bold py-2.5 rounded-lg border border-stone-200">
                        Continue as Guest
                    </a>
                    <a href="{{ route('home') }}" class="text-[11px] text-stone-400 hover:text-stone-500 transition font-light">← Cancel and Return to Homepage</a>
                </div>
            </form>
        </div>
    </div>
