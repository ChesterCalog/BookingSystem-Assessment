            ...
            <form action="{{ route('booking.store') }}" method="POST" class="space-y-8">
                    ...
                    <div class="pb-4 border-b border-stone-100">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="guest_name" required class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4">
                            </div>
                            <div>
                                <label class="block text-xs font-bold tracking-widest uppercase text-stone-600 mb-2">Email Address</label>
                                <input type="email" name="guest_email" required class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4">
                            </div>
                        </div>
                        <div class="mt-4">
                            <label class="block text-xs font-bold tracking-widest uppercase text-stone-600 mb-2">Contact Phone</label>
                            <input type="text" name="guest_phone" required placeholder="+63 912 345 6789" class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" name="check_in" required class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4">
                    </div>
                    <div>
                        <label class="block text-xs font-bold tracking-widest uppercase text-stone-600 mb-2">Check-out Date</label>
                        <input type="date" name="check_out" required class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-bold tracking-widest uppercase text-stone-600 mb-2">Select Accommodation</label>
                    <select id="room_select" name="room_type_id" required class="block w-full rounded-xl border-0 py-3.5 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm px-4 bg-white cursor-pointer">
                        <option value="" disabled selected>-- Choose a room --</option>
                        @foreach($rooms as $room)
                            <option value="{{ $room->id }}">
                                {{ $room->name }} - ${{ number_format($room->base_price, 2) }}/night
                            </option>
                        @endforeach
                    </select>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            selectElement.addEventListener('change', function() {
                ...
                if (room) {
                    ...
                    pImage.src = room.image ? '/storage/' + room.image : '';
                    pName.textContent = room.name;
                    pSize.textContent = room.size ?? '-- sq. ft.';
