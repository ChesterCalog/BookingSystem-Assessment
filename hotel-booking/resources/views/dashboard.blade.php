...
{{-- ── NAVBAR (unified with welcome.blade.php) ─────────────────────────── --}}
<nav class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-stone-100 px-6 lg:px-12 py-4 flex flex-col lg:flex-row justify-between items-center gap-4 lg:gap-0 shadow-sm">
    ...
    <div class="hidden md:flex items-center space-x-8 text-xs font-bold tracking-widest uppercase text-stone-600">
        <a href="{{ url('/') }}#amenities" class="hover:text-amber-800 transition">The Resort Experience</a>
        <a href="{{ url('/') }}#products" class="hover:text-amber-800 transition">Our Accommodations</a>
    </div>

    <div class="flex flex-wrap items-center justify-center gap-4 text-xs font-bold tracking-widest uppercase w-full lg:w-auto">
        
        {{-- Grouped Name and Avatar --}}
        <div class="flex items-center gap-3 px-3 py-2">
            ...
            <div class="w-8 h-8 rounded-full border-2 border-amber-600 overflow-hidden flex items-center justify-center bg-stone-100">
                @include('partials.avatar', ['avatar' => Auth::user()->avatar ?? 'male_1', 'size' => 32])
            </div>
        </div>

        @if(Auth::user()->role === 'staff' || Auth::user()->role === 'admin')
            <a href="{{ route('staff.portal') }}" class="text-stone-500 hover:text-stone-800 border border-stone-200 hover:border-stone-400 px-4 py-2.5 rounded-xl transition bg-stone-50/50">
                Staff Portal
            </a>
        @endif
...
<div class="page-wrapper">
            ...
            <div class="bookings-table-container">
                @if($bookings->isEmpty())
                    <div class="empty-state">
                        ...
                        <a href="{{ url('/') }}#products">Browse Accommodations</a> 
                    </div>
                @else
                    <table>
                        <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td class="td-id">{{ $booking->booking_code }}</td>
                                <td>
                                    {{-- Room number placeholder — adjust once room number is stored --}}
                                ...
                                <td class="td-date">{{ $booking->check_in->format('Y-m-d') }}</td>
                                <td class="td-date">{{ $booking->check_out->format('Y-m-d') }}</td>
                                <td class="td-amount">
    ...
    {{-- ── PROFILE AVATAR (hidden for guests) ────────────────────────── --}}
    ...
    <div class="avatar-section">
        <div class="card">
            ...
            <form method="POST" action="{{ route('dashboard.avatar') }}">
                @csrf
                <div class="avatar-grid">
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="male_1"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'male_1' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Male 1: Short tidy hair, collared --}}
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="male_2"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'male_2' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Male 2: Wavy side-parted hair --}}
                            <svg viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="36" cy="36" r="36" fill="#EDE8DF"/>
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="male_3"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'male_3' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Male 3: Curly hair, darker skin --}}
                            <svg viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="36" cy="36" r="36" fill="#EDE8DF"/>
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="female_1"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'female_1' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Female 1: Long straight hair --}}
                            <svg viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="36" cy="36" r="36" fill="#EDE8DF"/>
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="female_2"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'female_2' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Female 2: Bob cut with bangs --}}
                            <svg viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="36" cy="36" r="36" fill="#EDE8DF"/>
                    ...
                    <label class="avatar-option">
                        <input type="radio" name="avatar" value="female_3"
                            {{ (Auth::user()->avatar ?? 'male_1') === 'female_3' ? 'checked' : '' }}>
                        <div class="avatar-ring">
                            {{-- Female 3: Bun / updo, darker skin --}}
                            <svg viewBox="0 0 72 72" xmlns="http://www.w3.org/2000/svg">
                                <circle cx="36" cy="36" r="36" fill="#EDE8DF"/>
