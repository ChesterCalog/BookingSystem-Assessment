...
<body class="bg-[#FAF9F5] text-stone-800 antialiased min-h-screen flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div class="bg-white py-10 px-4 shadow-sm border border-stone-200/60 sm:rounded-3xl sm:px-10">
            ...
            <form method="POST" action="<?php echo e(route('staff.login.store')); ?>" class="space-y-6">
                <?php echo csrf_field(); ?>

                <div>
                    <label for="email" class="block text-xs font-bold tracking-widest uppercase text-stone-600">Employee Email</label>
                    <div class="mt-2">
                        <input id="email" name="email" type="email" value="<?php echo e(old('email')); ?>" required autofocus class="block w-full rounded-xl border-0 py-3 text-stone-900 shadow-sm ring-1 ring-inset ring-stone-300 placeholder:text-stone-400 focus:ring-2 focus:ring-inset focus:ring-amber-800 sm:text-sm sm:leading-6 px-4">
...
<?php /**PATH C:\Users\Licensed User\Documents\GitHub\BookingSystem-Assessment\hotel-booking\resources\views/auth/staff-login.blade.php ENDPATH**/ ?>